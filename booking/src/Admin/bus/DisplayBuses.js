...
function DisplayBuses() {
  ...
  const [busesList, setBuses] = useState(null);
  useEffect(() => {
    getBuses();
  }, []);
  const getBuses = () => {
    fetch("http://localhost:6000/api/buses/get_buses")
      .then((res) => res.json())
      .then(
        (result) => {
          setBuses(result);
  ...
  if (!busesList) return <div>No Buse found</div>;
  return (
    <>
            ...
            <div className="relative w-1/2 my-6 mx-auto max-w-3xl">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-blue-300 rounded-t ">
                  <h3 className="text-3xl font=semibold">Update Buse</h3>
                  <button
